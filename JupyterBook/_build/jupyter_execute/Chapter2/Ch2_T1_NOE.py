...
# 
# Here, we have to use a special tool in SLEEPY to apply a field in the lab frame. Keep in mind, longitudinal relaxation is driven by terms in the Hamiltonian that are dropped when we go to the rotating frame. However, in the lab frame, the applied RF fields are no longer time-independent and need to be explicitly simulated. SLEEPY uses several tricks for doing this. First, we get the time dependent field  based on two steps, and scaling up the field strength/adjusting the phase. Then, we get the propagator including the two-step RF in its eigenbasis and propagate either for one step of the rotor period, or for Dt for a static sequence.
# 
